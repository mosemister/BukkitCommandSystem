...
public class AnyArgument<A> implements CommandArgument<A> {
    ...
    private final @NotNull BiFunction<CommandContext, ArgumentContext, Collection<A>> supply;
     ...
     */
    public AnyArgument(@NotNull String id, @NotNull Function<A, String> toString, @NotNull BiFunction<Collection<A>, String, A> fromString, @NotNull BiFunction<CommandContext, ArgumentContext, @NotNull Collection<A>> supply) {
        this.id = id;
    ...
    public static AnyArgument<String> buildStringList(String id, BiFunction<CommandContext, ArgumentContext, Collection<String>> function) {
        return new AnyArgument<>(id, v -> v, (collection, value) -> collection.stream().anyMatch(v -> v.equalsIgnoreCase(value)) ? value : null, function);
    }

    public static <A> AnyArgument<A> buildMappedToString(String id, BiFunction<CommandContext, ArgumentContext, Collection<A>> function) {
        return buildMapped(id, Object::toString, function);
    }

    public static <A> AnyArgument<A> buildMapped(String id, Function<A, String> toString, BiFunction<CommandContext, ArgumentContext, Collection<A>> function) {
    ...
    public @NotNull CommandArgumentResult<A> parse(@NotNull CommandContext context, @NotNull ArgumentContext argument) throws ArgumentException {
        ...
        if (result == null) {
            throw new ArgumentException("Unknown value of " + arg);
        }
